...
The static-site scenario already supports `verify_site` and `render_site` for Hugo and currently defaults to `github.com/google/docsy@v0.17.0`. The site should use Hugo modules, Docsy config, Docsy navbar settings, and project SCSS overrides.
...
2. Keep Docsy `v0.13.0` behavior as the initial visual target.
   ...
   The request specifically calls out transparent navbar behavior like Docsy `v0.13.0`, and the prepared static-site scenario also defaults to `github.com/google/docsy@v0.17.0`. The implementation should start with Docsy `v0.13.0` unless there is a compatibility issue that forces a newer version.
