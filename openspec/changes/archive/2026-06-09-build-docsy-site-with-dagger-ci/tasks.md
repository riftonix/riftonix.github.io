...
- [x] 2.4 Copy the cover image into repository assets or static files.
...
- [x] 5.1 Add Renovate configuration for GitHub Actions, Go modules, and repository policy defaults.
- [x] 5.2 Add Renovate regex managers for standalone `DAGGER_VERSION`, Hugo pins, Docsy URL pins, and static-site scenario references not covered by standard managers.
- [x] 5.3 Add grouping, labels, and schedules to keep dependency update pull requests manageable.
- [x] 5.4 Validate Renovate configuration locally or with a dry-run command if credentials/tooling are available.

## 6. Validation

- [x] 6.1 Run OpenSpec validation for `build-docsy-site-with-dagger-ci`.
- [x] 6.2 Run local site verification through the Dagger static-site scenario.
- [x] 6.3 Render production output and confirm `https://riftonix.io/` base URL behavior.
- [x] 6.4 Render a sample preview output and confirm pull request path behavior.
- [x] 6.5 Inspect the rendered home page to confirm the background image and transparent navbar behavior.
- [x] 6.6 Check git status and summarize generated files and any follow-up operational steps.
- [x] 6.7 Before archiving this proposal, remove machine-local absolute paths from proposal/design/spec/task artifacts or replace them with stable repository-relative paths, public repository links, or implementation notes.
