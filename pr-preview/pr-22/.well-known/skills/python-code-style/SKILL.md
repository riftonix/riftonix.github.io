...
# Configure in pyproject.toml for a single Python project
[tool.ruff]
line-length = 120
...
For a single Python project, place Ruff settings in that project's `pyproject.toml`:
...
For a monorepo with multiple Python projects under a shared subtree, keep shared Ruff settings in one
standalone config at the shared subtree root, such as `modules/ruff.toml`. Do not create a repository-root
`pyproject.toml` only for Ruff if the repository root is not itself a Python project. Do not duplicate the
same Ruff settings in every leaf project unless those projects intentionally diverge.

Standalone `ruff.toml` uses top-level Ruff sections, not `[tool.ruff]`:
...
```bash
ruff check --no-cache --fix .  # Lint and auto-fix
...
2. **Use one shared style config per Python scope** - In monorepos, prefer a shared `ruff.toml` at the Python subtree root
...
10. **Automate in CI** - Run linters on every commit
